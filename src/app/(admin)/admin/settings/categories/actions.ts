...
export const createCategory = async (formData: FormData) => {
  ...
  const { error } = await createClient("server_action").from("categories").insert({ title, color });
...
export const deleteCategory = async (id: string) => {
  ...
  console.log("wtf", id);
  const { error } = await createClient("server_action").from("categories").delete().eq("id", id);
  if (error) throw new Error("Bruhx2");
  console.log(error);
