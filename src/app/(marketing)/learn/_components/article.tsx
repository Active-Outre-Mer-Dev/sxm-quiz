...
import type { Article, Profile } from "@/types/custom.types";

type PropTypes = {
  profiles: Profile | null;
} & Article;

export function Article(props: PropTypes) {
  return (
    <Link
      href={`/learn/${props.category}/${props.slug}`}
      className="overflow-hidden group lg:basis-1/3 grow"
    >
      <figure
  ...
  justify-center  `}
    ...
    duration-200 ease-out`}
      ...
      <Author
        firstName={props.profiles?.first_name}
        lastName={props.profiles?.last_name}
        img={props.profiles?.profile_image || ""}
      />
    </Link>
  );
}
